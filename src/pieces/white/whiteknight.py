...
class White_Knight(object):
  def __init__(self):
    self.img = [pg.image.load('../assets/sprites/whiteKnight.png') for x in range(2)]
  ...
  def update_movelist(self):
    for i in range(2):
      ...
      if self.row[i]-2 >= 0 and self.col[i]-1 >= 0:
        if White.blocks[self.row[i]-2][self.col[i]-1] == 0:
          self.movelist[i].append((self.row[i]-2, self.col[i]-1))
        else:
      ...
      if self.row[i]-1 >= 0 and self.col[i]-2 >= 0:
        if White.blocks[self.row[i]-1][self.col[i]-2] == 0:
